package com.knoldus.question3.Integration_Testing
import com.knoldus.question3.impl.{CompanyImpl, EmailImpl, UserImpl}
import com.knoldus.question3.db.CompanyReadDto
import com.knoldus.question3.validator.{CompanyValidator, EmailValidator, UserValidator}
...
class IT_Validator extends AsyncFlatSpec {
  ...
  "Email Id " should "be Valid" in {
    val result = companyImpl.getEmailId("praanav@gmail.net")
    assert(result == "valid")
  }

  "Email Id with wrong Recipient name " should "be invalid" in {
    val result = companyImpl.getEmailId("!!praanav@gmail.com")
  ...
  " A email " should "be Invalid without @ symbol" in {
    val result = userImpl.getEmailId("praanav15.com")
    assert(result == "invalid")
  }

  "A email " should "be Invalid wrong domain name" in {
    val result = userImpl.getEmailId("praanav.com")
    assert(result == "invalid")
  }
  ...
  "A email " should "Invalid with wrong Top-Level Domain Name " in {
    val result = emailImpl.getEmailId("praanav.ccom")
    assert(result=="no")
  }
}
